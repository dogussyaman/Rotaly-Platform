  ...
  return (
    <div className="flex flex-col h-[400px] bg-background border border-border rounded-3xl overflow-hidden shadow-sm">
      <div className="px-5 py-3 border-b border-border bg-muted/30 flex justify-between items-center">
        <h4 className="text-sm font-bold">Ev Sahibi ile Mesajlaş</h4>
        <Button variant="ghost" size="sm" onClick={onClose} className="text-xs h-7 rounded-full">Kapat</Button>
      </div>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3">
        {loading ? (
          ...
          <div className="text-center py-10">
            <p className="text-xs text-muted-foreground italic">Henüz bir mesaj yok. İlk mesajı siz gönderin!</p>
          </div>
        ) : (
          messages.map((msg) => (
            <div 
